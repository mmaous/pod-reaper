...
func TestLoadDefaults(t *testing.T) {
	...
	if cfg.PendingThreshold != 2*time.Minute {
		t.Errorf("expected PendingThreshold=2m, got %v", cfg.PendingThreshold)
	}
	if cfg.TerminatingThreshold != 2*time.Minute {
		t.Errorf("expected TerminatingThreshold=2m, got %v", cfg.TerminatingThreshold)
	}
	if cfg.NotReadyThreshold != 2*time.Minute {
		t.Errorf("expected NotReadyThreshold=2m, got %v", cfg.NotReadyThreshold)
	}
